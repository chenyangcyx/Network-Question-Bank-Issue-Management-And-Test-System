...
<?php
    ...
    if (isset ( $_SESSION ["code"] )) {//判断code存不存在，如果不存在，说明异常登录
        ...
        $bookinfo=mysqli_fetch_array($result);
        if($bookinfo){
          mysqli_query($con,"update teacher_course set course_id='{$course_id}' where teacher_id='{$teacher_id}'") or die ( "选择课程失败，请检查你的内容！".'<br>'."错误信息：". mysqli_error ($con));
        }else{
          mysqli_query($con,"insert into teacher_course values('{$teacher_id}','{$course_id}');") or die ( "选择课程失败，请检查你的内容！".'<br>'."错误信息：". mysqli_error ($con));
        }
        ?>
