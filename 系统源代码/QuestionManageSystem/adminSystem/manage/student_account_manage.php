...
<div class="container">
	...
	<div class="row clearfix">
		...
		<div class="col-md-3 column">
			 ...
			 <button type="button" class="btn btn-default btn-block btn-warning" onclick="window.location.href='student_account_manage.php'">学生账号</button>
		...
		<div class="col-md-7 column">
                <?php
                    $result=mysqli_query($con,"select student_info.*,student_account.* from student_info,student_account where student_info.student_username=student_account.student_username");
                    if($result && mysqli_num_rows($result)){
						$num=1;
						echo '<p><strong>所有的学生信息如下：</strong></p><table class="table table-hover table-bordered">';
                        ...
                        while($row=mysqli_fetch_assoc($result)){
                            ...
                            echo '<td>';
                            echo '<button type="button" class="btn btn-default btn-block btn-success btn-xs" onclick="window.location.href=\'student_reset.php?id='.$row['student_username'].'\''.'">重置账号</button>';
                            echo '</td>';
                            echo '<td>';
                            echo '<button type="button" class="btn btn-default btn-block btn-danger btn-xs" onclick="window.location.href=\'student_delete.php?student_username='.$row['student_username'].'&student_id='.$row['student_id'].'\''.'">删除账号</button>';
                            echo '</td>';
                            echo '</tr>';
                            $num++;
						}
